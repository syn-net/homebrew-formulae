# brew install --with-fastcgi --with-mysql --with-suhosin -v
...
class Php < Formula
  ...
  def options
   [
     ...
     ['--with-fastcgi', 'Enable building of the FastCGI executable (implies --without-apache)'],
     ['--with-force-cgi-redirect', 'Reference: http://techtrunch.com/installations/compile-apache-suphp-ubuntu#more-1236'],
  ...
  def install
    ...
    args.push "--enable-force-cgi-redirect" if ARGV.include? '--with-force-cgi-redirect'
    ...
    elsif ARGV.include? '--with-fastcgi'
      args.push "--with-fastcgi=#{prefix}"
    end

    # Build Apache module by default
    unless ARGV.include? '--with-fpm' or ARGV.include? '--with-fastcgi' or ARGV.include? '--without-apache'
    ...
    unless ARGV.include? '--without-apache' or ARGV.include? '--with-fastcgi' or ARGV.include? '--with-fpm'
